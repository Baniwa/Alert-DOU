    ...
    <div className="max-w-4xl w-full mx-auto">
      ...
      <div className="mb-8 border-b border-gray-200 pb-6">
        ...
        <p className="text-sm text-gray-600 mt-2 max-w-2xl leading-relaxed">
          Cadastre os concursos que você acompanha. O sistema identifica automaticamente publicações relevantes nos resumos do DOU.
        </p>
      </div>
      ...
      {concursos.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-8">
          {concursos.map((kw) => (
            <span key={kw} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#1351B4]/10 text-[#1351B4] text-[12px] font-bold rounded-lg border border-[#1351B4]/20">
              <Search size={11} />
              {kw}
              <button onClick={() => remove(kw)} className="ml-1 hover:text-red-500 transition-colors" title={`Remover ${kw}`}>
                <X size={12} />
              </button>
            </span>
          ))}
      ...
      {concursos.length === 0 && (
        <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-16 text-center">
          ...
          <p className="text-sm text-gray-500 max-w-sm mx-auto">
            Adicione palavras-chave acima para que o sistema busque publicações relevantes nos resumos do DOU.
          </p>
        </div>
      )}
        ...
        <>
          ...
          {!isLoading && matches.length === 0 && (
            <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center">
              <p className="text-gray-500 text-sm mb-1">Nenhum resumo disponível menciona os concursos monitorados.</p>
              <Link to="/editions" className="inline-flex items-center gap-1.5 mt-4 text-[12px] font-bold text-[#1351B4] hover:text-[#168821] transition-colors">
                Ver edições do DOU <ArrowRight size={13} />
              </Link>
            </div>
          )}
          ...
          {!isLoading && matches.length > 0 && (
            <div className="space-y-4">
              {matches.map(({ summary, matched }) => (
                <div key={summary.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5 hover:border-gray-300">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    ...
                    <div className="flex flex-wrap gap-1 justify-end flex-shrink-0">
                      {matched.map((kw) => (
                        <span key={kw} className="text-[10px] font-bold px-2 py-0.5 bg-[#168821]/10 text-[#168821] rounded-full border border-[#168821]/20">{kw}</span>
                      ))}
                    </div>
                  </div>
                  <p className="text-[13px] text-gray-600 leading-relaxed line-clamp-3 mb-4">{summary.summary.replace(/[*#]/g, '')}</p>
                  <Link to={`/editions/${summary.editionId}`} className="inline-flex items-center gap-1.5 text-[12px] font-bold text-[#1351B4] hover:text-[#168821] transition-colors">
                    Ler resumo completo <ArrowRight size={13} />
                  </Link>
                </div>
              ))}
            </div>
